...
# Version - 0.0.2
# Description - Implementation of Digital Signature using RSA
...
# gcd return the greatest common divisor of the two nos using Euclidean Algorithm
def gcd(a, b):
    a = abs(a)
    b = abs(b)
...
# getRandE return the value of e such that e is co-prime of O(n)
def genRandE(phin):
    e = 65537

    g = gcd(e, phin)
    # checking if e and )(n) is co-prime
    while g != 1:
...
def encrypt():
    ...
    # if p = q then generate another p and q
    while p == q:
        p, q = genPrime(512), genPrime(512)
    ...
    digest = hashlib.sha224(msg.encode('ascii')).hexdigest()
    ...
    # encrypting the digest using RSA algorithm CT = M^d mod n
...
def decrypt():
    ...
    # calculating the length of message + digest
    l = len(msg)
    ...
    # getting the length of encrypted digest
    start = (l - int(start) + 1) * -1

    # separating digest from message
    digest = msg[start:l]
    ...
    msg = hashlib.sha224(msg.encode('ascii')).hexdigest()
...
if __name__ == '__main__':
    # increasing the memory allocation to accommodate calculations and storage of very large numbers
